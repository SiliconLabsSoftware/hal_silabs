...
def write_header(path: Path, family, peripherals: dict, abuses: list) -> None:
  ...
  lines = [
    "/**",
    " * @file",
    f" * @brief Devicetree pin control helpers for Silicon Labs {family_display}",
    ...
    "",
    "/**",
    f" * @defgroup pinctrl_{family} Silicon Labs {family_display} pin control helpers",
    f" * @brief Macros for pin control configuration of Silicon Labs {family_display}",
  ...
  for peripheral in sorted(peripherals.values(), key=lambda p: p.name):
    ...
    sig_list = ", ".join(f"@c {s}" for s in sig_names)
    lines.append(f" * - @c {peripheral.name}: {sig_list}")
  ...
  description = [
    f"Pin Control for Silicon Labs {family_display} devices",
    "",
  ...
  outfile = path / f"{family}-pinctrl.h"
